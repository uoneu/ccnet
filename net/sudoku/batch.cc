...
InputPtr readInput(std::istream& in)
{
  ...
  while (getline(in, line))
  {
    ...
    */
    if (!line.empty()) input->push_back(line.c_str());
    /*
    cout << line << endl;
    cout << line.size() << endl;
    break;
...
void done(const string& name, double elapsed, int count)
{
  LOG_INFO << name << " " << elapsed << " seconds "
           ...
           << " us per request.";
  ++g_finished;
  LOG_INFO << "[g_finished:g_connections" << g_finished << " " << g_connections ;
...
//  在一个loop内同时启动多个客户连接
void runClient(std::ifstream& in, const InetAddress& serverAddr, int conn)
{
